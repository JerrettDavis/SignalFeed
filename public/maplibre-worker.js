...
var H = class {
    ...
    update(e, t, n) {
      for (let t of e) {
        ...
        ((e._featureFilter = I(e.filter, `layers[${t.id}].filter`, n)),
      ...
      let r = z(Object.values(this._layerConfigs), this.keyCache);
      for (let e of r) {
...
var W = class {
  ...
  async parse(e, t, r, i, a) {
    ((this.data = e), (this.collisionBoxArray = new ee()));
    let o = new x(Object.keys(e.layers).sort()),
      s = new b(this.tileID, this.promoteId);
    s.bucketLayerIDs = [];
    let l = {},
      u = {
        featureIndex: s,
    ...
    for (let t in f) {
      ...
      for (let e = 0; e < n.length; e++) {
        let r = n.feature(e),
          o = s.getId(r, t);
        a.push({ feature: r, id: o, index: e, sourceLayerIndex: i });
      }
      for (let e of f[t]) {
        let t = e[0];
        (t.source !== this.source &&
          ...
          !t.isHidden(this.zoom, !0) &&
            ...
            })).populate(a, u, this.tileID.canonical),
            s.bucketLayerIDs.push(e.map((e) => e.id))));
      }
    }
    let p = R(u.glyphDependencies, (e) => Object.keys(e).map(Number));
    ...
    let S = u.dashDependencies,
      C = Promise.resolve({});
    if (Object.keys(S).length) {
      let e = new AbortController();
      (this.inFlightDependencies.push(e),
        (C = i.sendAsync({ type: `GDA`, data: { dashes: S } }, e)));
    }
    let [w, T, E, D] = await Promise.all([h, _, y, C]),
      O = new U(w),
      k = new d(T, E);
    for (let e in l) {
      let t = l[e];
      t instanceof n
        ? (G(t.layers, this.zoom, r),
          M({
            bucket: t,
            glyphMap: w,
            glyphPositions: O.positions,
            imageMap: T,
            imagePositions: k.iconPositions,
            showCollisionBoxes: this.showCollisionBoxes,
            canonical: this.tileID.canonical,
            subdivisionGranularity: u.subdivisionGranularity,
          }))
        : t.hasDependencies &&
          (t instanceof j || t instanceof c || t instanceof m) &&
          (G(t.layers, this.zoom, r),
          t.addFeatures(u, this.tileID.canonical, k.patternPositions, D));
    }
    return {
      buckets: Object.values(l).filter((e) => !e.isEmpty()),
      featureIndex: s,
      collisionBoxArray: this.collisionBoxArray,
      glyphAtlasImage: O.image,
      imageAtlas: k,
      dashPositions: D,
      glyphMap: this.returnDependencies ? w : null,
      iconMap: this.returnDependencies ? T : null,
      glyphPositions: this.returnDependencies ? O.positions : null,
    };
  }
};
function G(e, t, n) {
...
function ie(e, t, n) {
  ...
  for (let t = 0; t < e.length; t++) {
    ...
    ((c = w(c, n.type, -128, -128, r + 128, r + 128)),
      c.length !== 0 && s.push(new J(n.type, c, n.properties, n.id, r)));
  }
  return new Y(s, e.name, r);
}
var ae = class {
    constructor(e, t, n) {
      ((this.actor = e),
        ...
        (this.tileState = new K()),
        (this.overzoomedTileResultCache = new te(1e3)));
    }
    loadVectorTile(t, n) {
      try {
        return {
          vectorTile: t.encoding === `mlt` ? new D(n) : new F(new e(n)),
          rawData: n,
        };
      } catch (e) {
    ...
    async loadTile(e) {
      ...
      let i = this._startRequestTiming(e),
        a = new W(e);
      this.tileState.startLoading(t, a);
      let o = new AbortController();
      a.abort = o;
      try {
        let s = await r(e.request, o);
        if (e.etag && e.etag === s.etag)
          return (
            this.tileState.finishLoading(t),
            this._getEtagUnmodifiedResult(s, i)
          );
        ...
        let d = this._getExpiryData(s),
          f = this._finishRequestTiming(i);
        ((a.vectorTile = l), this.tileState.markLoaded(t, a));
        let p = { rawData: u, cacheControl: d, resourceTiming: f };
        return (
          this.tileState.setParsing(t, p),
          await this._parseWorkerTile(a, e)
        );
      } catch (e) {
        throw (
          this.tileState.finishLoading(t),
          this.tileState.markLoaded(t, a),
          e
        );
      }
    }
    ...
    async _parseWorkerTile(e, t) {
      ...
      if (n) {
        ...
        ((r = P({ rawTileData: i.slice(0), encoding: s }, r, a, o)),
    ...
    _startRequestTiming(e) {
      if (e.request?.collectResourceTiming) return new q(e.request.url);
    }
    ...
    _getOverzoomTile(e, t) {
      ...
      let u = { vectorTile: c, rawData: O(c).buffer };
  ...
  oe = class {
    ...
    async loadTile(e) {
      let {
        ...
        l = r.width + 2,
        u = r.height + 2,
        d = A(r) ? new i({ width: l, height: u }, await S(r, -1, -1, l, u)) : r,
        f = new y(t, d, n, a, o, s, c);
  ...
  se = class {
    ...
    loadVectorTile(e) {
      ...
      if (!a) return null;
      let o = new ne(a.features, { version: 2, extent: h });
      return { vectorTile: o, rawData: O(o, t).buffer };
    }
    async loadTile(e) {
      let { uid: t } = e,
        n = new W(e);
    ...
    async _parseWorkerTile(e, t) {
      ...
      if (n) {
        let { rawData: t } = n;
        ((r = P({ rawTileData: t.slice(0), encoding: `mvt` }, r)),
          this.tileState.removeParsing(e.uid));
      }
      return r;
    }
    ...
    async loadData(e) {
      ...
      try {
        ...
        if ((delete this._pendingRequest, !f(e))) throw e;
        return { abandoned: !0 };
      }
    }
    _startRequestTiming(e) {
    ...
    async loadAndProcessGeoJSON(e, t) {
      if ((e.request && (e.data = (await u(e.request, t)).data), e.data)) {
...
function ce(e, t) {
  let n = P(t.geojsonVtOptions || {}, { updateable: !0, clusterOptions: Z(t) });
  return new _(e, n);
}
function Z({ geojsonVtOptions: e, clusterProperties: t, source: n }) {
  ...
  for (let e of s) {
    let [a, o] = t[e],
      s = k(o, `sources.${n}.clusterProperties.${e}[1]`),
      c = k(
...
var $ = class {
  ...
  _getLayerIndex(e) {
    ...
    return ((t ||= this.layerIndexes[e] = new H()), t);
  }
