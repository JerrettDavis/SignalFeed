import type { SightingFlair, SightingFlairAssignmentMethod } from "@/domain/flairs/sighting-flair";
import type { FlairSuggestion, FlairSuggestionId, FlairSuggestionStatus } from "@/domain/flairs/sighting-flair";
...
export interface SightingFlairRepository {
  ...
  updateSuggestionVotes(id: FlairSuggestionId, voteCount: number): Promise<void>;
  updateSuggestionStatus(id: FlairSuggestionId, status: FlairSuggestionStatus): Promise<void>;
  getPendingSuggestions(): Promise<FlairSuggestion[]>;
  getUserSuggestion(sightingId: SightingId, flairId: FlairId, userId: string): Promise<FlairSuggestion | null>;
}
